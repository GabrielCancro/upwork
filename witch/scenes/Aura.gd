...
func _ready():
	...
	if Global.options.aura_date != Global.now_date(): Global.options.selected_aura = 0
	check_aura_selcted()

func check_aura_selcted():
	if Global.options.selected_aura == 0: return
	state = "stoped"
	$Label_once.visible = true
	if Global.options.language=="en": $Label_once.text = "Once a day"
	if Global.options.language=="ru": $Label_once.text = "Один раз в день"
	$TextureText.visible = false
	$btn_hold.visible = false
	$Aura2.visible = false
	$Aura1.texture = load("res://assets/aura/aura_"+str(Global.options.selected_aura)+".png")
	$Meaning.texture = load("res://assets/meaning/meaning_color_"+str(Global.options.selected_aura-1)+".png")
	print("LOAD LOAD ","res://assets/meaning/meaning_text_"+str(Global.options.selected_aura-1)+"_"+Global.options.language+".png")
	$Meaning/Text_image.texture = load("res://assets/meaning/meaning_text_"+str(Global.options.selected_aura-1)+"_"+Global.options.language+".png")
...
func onHoldDown():
	...
	while state=="changing":
		...
		$Aura2.modulate.a = 0
		var rnd = randi()%6+1
		if rnd==Global.options.selected_aura: rnd = randi()%6+1
		Global.options.selected_aura = rnd
		...
		$Tween.start()
		$Aura2.texture = load("res://assets/aura/aura_"+str(Global.options.selected_aura)+".png")
...
func onHoldUp():
	...
	state = "stoped"
	Global.options.aura_date = Global.now_date()
